...
public class BlockEffectsResolverV1_21_5
implements BlockEffectsResolver {
        ...
        for (GrimPlayer.Movement movement : movements) {
            ...
            if (movement.axisIndependant() && to.lengthSquared() > 0.0) {
                for (Collisions.Axis axis : BlockCollisions.axisStepOrder(to)) {
                    ...
                    BlockEffectsResolverV1_21_5.checkInsideBlocks(player, from, vector, visitedBlocks);
                    from = vector;
                }
                continue;
            }
            BlockEffectsResolverV1_21_5.checkInsideBlocks(player, movement.from(), movement.to(), visitedBlocks);
        }
        visitedBlocks.clear();
    }
    ...
    private static void checkInsideBlocks(GrimPlayer player, Vector3d from, Vector3d to, LongSet visitedBlocks) {
        ...
        BlockEffectsResolverV1_21_5.forEachBlockIntersectedBetween(from, to, boundingBox, (blockPos, i) -> {
    ...
    private static void forEachBlockIntersectedBetween(Vector3d start, Vector3d end, SimpleCollisionBox boundingBox, BlockStepVisitor blockStepVisitor) {
        ...
        if (!(direction.lengthSquared() < (double)GrimMath.square(0.99999f))) {
            ...
            int iterationCount = BlockEffectsResolverV1_21_5.addCollisionsAlongTravel(alreadyVisited, subtractedMinPosition, boxMinPosition, boundingBox, blockStepVisitor);
            for (Vector3i blockPos : SimpleCollisionBox.betweenClosed(boundingBox)) {
                if (alreadyVisited.contains(GrimMath.asLong(blockPos))) continue;
            ...
            for (int x = currentX; x <= endX; ++x) {
                for (int y = currentY; y <= endY; ++y) {
                    for (int z = currentZ; z <= endZ; ++z) {
                        if (!alreadyVisited.add(GrimMath.asLong(x, y, z))) continue;
