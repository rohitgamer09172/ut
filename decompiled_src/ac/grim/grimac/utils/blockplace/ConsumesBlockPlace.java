...
public final class ConsumesBlockPlace {
    public static boolean consumesPlace(@NotNull GrimPlayer player, @NotNull WrappedBlockState state, @NotNull BlockPlace place) {
        if (state.getType() == StateTypes.BELL) {
            return ConsumesBlockPlace.goodBellHit(state, place);
        }
        ...
        if (state.getType() == StateTypes.TNT && (place.itemStack.getType() == ItemTypes.FIRE_CHARGE || place.itemStack.getType() == ItemTypes.FLINT_AND_STEEL)) {
            player.compensatedWorld.updateBlock(place.position, StateTypes.AIR.createBlockState(CompensatedWorld.blockVersion));
            return true;
        }
        ...
        if (state.getType() == StateTypes.COMMAND_BLOCK || state.getType() == StateTypes.CHAIN_COMMAND_BLOCK || state.getType() == StateTypes.REPEATING_COMMAND_BLOCK || state.getType() == StateTypes.JIGSAW || state.getType() == StateTypes.STRUCTURE_BLOCK) {
    ...
    private static boolean goodBellHit(@NotNull WrappedBlockState bell, @NotNull BlockPlace place) {
        ...
        return place.hitData != null && ConsumesBlockPlace.isProperHit(bell, direction, place.hitData.getRelativeBlockHitLocation().getY());
    }
    ...
    private static boolean isProperHit(@NotNull WrappedBlockState bell, @NotNull BlockFace direction, double p_49742_) {
        if (direction == BlockFace.UP) return false;
        if (direction == BlockFace.DOWN) return false;
        if (p_49742_ > (double)0.8124f) return false;
        int n = 0;
        block5: do {
            switch (n == 0 ? 1.$SwitchMap$com$github$retrooper$packetevents$protocol$world$states$enums$Attachment[bell.getAttachment().ordinal()] : n) {
                default: {
                    throw new IncompatibleClassChangeError();
                }
                case 1: {
                    if (!AxisUtil.isSameAxis(bell.getFacing(), direction)) return false;
                    n = 4;
                    continue block5;
                }
                case 2: 
